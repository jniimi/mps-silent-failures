"""torch の版ごとの走査結果を 1 つの表にまとめる(版 × 条件の分類表)。
    ...
    uv run python versions.py --host-tag studio [--runs RUN_ID,...]
...
出力: results/summary/<host-tag>/versions.md
"""
...
def load(host: str, runs: list[str] | None, device: str):
    d = ROOT / "results" / "raw" / host
    mans = {}
...
def main():
    ...
    args = ap.parse_args()
    data = load(args.host_tag, args.runs.split(",") if args.runs else None, args.device)
    vs = list(data)
    ...
    out = ROOT / "results" / "summary" / args.host_tag / "versions.md"
    out.write_text("\n".join(L) + "\n")
    print(f"-> {out}")
