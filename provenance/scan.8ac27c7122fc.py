...
def worker_env(args) -> dict:
    ...
    out = sh(*worker_cmd(args)[:-3], "-c",
             "import json,platform,sys,torch;print(json.dumps({'python':platform.python_version(),"
             "'torch':torch.__version__,'executable':sys.executable}))")
    try:
...
def mem_est(spec: dict) -> int:
    """device 上の a, b(slice は 1 バッチ多い), out + CPU 側(torch 本体・生成/比較のチャンク)3GB。
    実測(M2 Ultra, torch 2.14)では bT / aT でも連続化のコピーは作られず driver_allocated ≈ a+b+out。"""
    M, K, N, B = spec["M"], spec["K"], spec["N"], spec["B"]
    s = ITEMSIZE[spec["dtype"]]
    pad = 1 if spec["layout"] == "slice" else 0
    return int(((B + pad) * (M * K + K * N) + B * M * N) * s * 1.05) + 3 * 2**30
...
def fingerprint(spec: dict, env: dict, code: dict, wenv: dict) -> str:
    key = {"spec": spec, "env": {k: env.get(k) for k in ("macos", "macos_build", "hw_model", "mem_bytes")},
...
class Runner:
    def __init__(self, args, run_id: str):
        ...
        self.limit = int(self.env["mem_bytes"] * args.mem_frac)
    ...
    def write_manifest(self, status: str, extra: dict | None = None) -> None:
        ...
        m = {**old, "run_id": self.run_id, "host_tag": self.args.host_tag, "status": status,
             ...
             "mem_limit_bytes": self.limit, **(extra or {})}
    ...
    def run(self, spec: dict, phase: str, probe: str | None = None, skip: str | None = None) -> dict:
        ...
        row = {"run_id": self.run_id, "fingerprint": fp, "host_tag": self.args.host_tag,
               ...
               "mem_est_bytes": mem_est(spec), "mem_limit_bytes": self.limit}
        if skip:
            row.update(cls=skip)
        elif row["mem_est_bytes"] > self.limit:
...
def main() -> None:
    ...
    ap.add_argument("--mem-frac", type=float, default=0.6, help="メモリ見積もりの上限(物理メモリ比)")
    ...
    if args.dry_run:
        dry_run(args, dtypes, layouts, shapes, idx_kinds)
        return
    if args.summary:
    ...
    print(f"# run_id={run_id} {R.env} worker={R.wenv} code={R.code['git_rev'][:10]}"
          f"{' (dirty)' if R.code['git_dirty_code'] else ''} limit={R.limit/2**30:.0f}GiB", flush=True)
                    ...
                    for B in final_points(infos[(s["id"], dt, lay)]):
                        for sd in FINAL_SEEDS:
                            R.run(make_spec(s["id"], dt, lay, "random", B, sd, "full", tol[dt], args.device), "final",
                                  skip="skipped_budget" if over() else None)
                        for kd in (idx_kinds if base else []):
...
def dry_run(args, dtypes, layouts, shapes, idx_kinds) -> None:
    """探索点・index 符号化・較正の実行数と時間の見積もり。二分探索と最終点は切り替わりの数による(前回の結果から見積もる)。"""
    env = host_env()
    limit = env["mem_bytes"] * args.mem_frac
    print(f"# {env}  上限 {limit/2**30:.0f} GiB ({args.mem_frac:.0%})")
    n = defaultdict(int)
    t = defaultdict(float)
    n_skip = 0
    for s in shapes:
        ...
        for dt in dtypes_for(s, dtypes, args):
            ...
            for lay in layouts:
                for B, _ in pts:
                    sp = make_spec(s["id"], dt, lay, "random", B, 0, "full" if B == cal_B(s) else "sample", 0, "mps")
                    if mem_est(sp) > limit:
                        n_skip += 1
                        continue
                    n["probe"] += 1
                ...
                if s["base"]:
                    for kd in idx_kinds:
                        for B, _ in idx_points(s, dt):
                            n["idx"] += 1
                            t["idx"] += time_est(make_spec(s["id"], dt, lay, kd, B, 0, "sample", 0, "mps"))
                # 最終点: 切り替わり 1 つ(2 点)と仮定、B は最大の境界付近
                Bf = max(b for b, _ in pts)
                for _ in range(2):
                    for sd in FINAL_SEEDS:
                        n["final"] += 1
                        t["final"] += time_est(make_spec(s["id"], dt, lay, "random", Bf, sd, "full", 0, "mps"))
                    if s["base"]:
                        for kd in idx_kinds:
                            n["final"] += 1
                            t["final"] += time_est(make_spec(s["id"], dt, lay, kd, Bf, 0, "full", 0, "mps"))
                n["bisect"] += 3
                t["bisect"] += 3 * time_est(make_spec(s["id"], dt, lay, "random", Bf // 2, 0, "sample", 0, "mps"))
...
def write_summary(host_tag: str, run_id: str) -> None:
    ...
    L = [f"# 走査結果の要約: {run_id}", "",
         ...
         f"- 環境: macOS {env['macos']} ({env['macos_build']}) / {env['chip']} ({env['hw_model']}) / "
         f"{env['mem_bytes']/2**30:.0f} GiB / device {man.get('device')} / torch {wk.get('torch')} / Python {wk.get('python')}",
         f"- コード: git {code['git_rev'][:12]}{'(scan/worker に未 commit の変更あり)' if code['git_dirty_code'] else ''}"
