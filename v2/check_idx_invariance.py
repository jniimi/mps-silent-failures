"""index 符号化入力で「規則は誤答を予測するが実際は正しかった」実行が、入力の不変性で説明できるかを CPU だけで確かめる。
    ...
    uv run --no-project --python 3.12 --with torch==2.14.0 --with numpy python check_idx_invariance.py [--run RUN_ID] [--results ../v1/results]
...
def main() -> None:
    ...
    if any(r.get("scale_log2") for r in rows):
        raise SystemExit("scale_log2 付き(テスト用の縮小 run)の行は対象外")
