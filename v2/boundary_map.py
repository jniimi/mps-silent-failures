"""走査結果から境界マップの図を作る(基本 shape、乱数入力、1 つの run)。
    ...
    uv run --no-project --python 3.12 --with matplotlib python boundary_map.py [--run RUN_ID] [--dtype fp32] [--results ../v1/results]
...
出力: <repo>/arxiv-v2/figs/fig_boundary_map.pdf(--results の既定は v2/results。v1 の走査から描くときは ../v1/results を渡す)
"""
...
def main() -> None:
    ...
    rows = [json.loads(l) for l in open(Path(args.results) / "raw" / host / f"{args.run}.jsonl")]
    cls = defaultdict(set)
    for r in rows:
    ...
    out = ROOT.parent / "arxiv-v2" / "figs" / "fig_boundary_map.pdf"
